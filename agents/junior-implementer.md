...
description: "Default agent for routine, well-scoped implementation that follows established patterns. Dispatch for localized feature additions with clear specs, straightforward bug fixes with identified root causes, UI/components mirroring existing patterns, CRUD or API-wiring work, small single-module refactors, or executing a design already produced by senior-implementer. Prefer over senior-implementer when the path forward is clear and the work is mainly convention-following. When dispatching, the task scope MUST be stated explicitly (files/modules in scope, what to change, what to leave untouched)."
model: sonnet
color: green
---
